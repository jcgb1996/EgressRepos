...
namespace Egress.Aplicacion.Services.Services.General
{
    public class Aes256
    {
        ...
        public string JwtEncriptarPassword(string Password)
        {
            ...
            var LlavesAES = WebConfigurationManager.AppSettings["LlaveAES"].ToString();
            var VectorAES = WebConfigurationManager.AppSettings["VectorAES"].ToString();

            byte[] Key = Encoding.ASCII.GetBytes(LlavesAES);
            byte[] IV = Encoding.ASCII.GetBytes(VectorAES);
        ...
        public string JwtDecrypPassword(string PasswordEcriptado)
        {
            ...
            var Password = ObtenerByteDeLosDatos(PasswordEcriptado);
            byte[] Key = Encoding.ASCII.GetBytes(LlavesAES);
            byte[] IV = Encoding.ASCII.GetBytes(VectorAES);
            using (Aes myAes = Aes.Create())
            {
                ...
                TextoPlano = DecryptStringFromBytes_Aes(Password, myAes.Key, myAes.IV);
            }
            return TextoPlano;
        }
